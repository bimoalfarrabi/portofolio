...
function buildOrbitLinks(projects) {
    ...
    const coords = projects.map((p) => ({ x: parseFloat(p.x), y: parseFloat(p.y) }));
    const links = [];
    const seen = new Set();
    ...
    coords.forEach((node, i) => {
        ...
        distances.forEach(({ j }) => {
            ...
            if (!seen.has(key)) {
                seen.add(key);
                links.push({ key, x1: `${coords[i].x}`, y1: `${coords[i].y}`, x2: `${coords[j].x}`, y2: `${coords[j].y}` });
            }
        });
    });
    return links;
...
function getDynamicSize(total, isFeatured) {
    if (total <= 3) return isFeatured ? 'size-28 sm:size-32 lg:size-36' : 'size-20 sm:size-24 lg:size-26';
    if (total <= 6) return isFeatured ? 'size-22 sm:size-26 lg:size-28' : 'size-16 sm:size-18 lg:size-20';
    if (total <= 9) return isFeatured ? 'size-18 sm:size-22 lg:size-24' : 'size-14 sm:size-16 lg:size-18';
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className={`frame relative md:hidden ${
            ...
            >
                ...
                <span className={`absolute right-4 top-3 font-mono text-[10px] uppercase tracking-[0.2em] ${isDark ? 'text-white/40' : 'text-ink-faint'}`}>
                    N={totalProjects}
                </span>
                <div className={`mt-8 divide-y ${isDark ? 'divide-white/10' : 'divide-line/60'}`}>
                    {orbitProjects.map((project, idx) => (
                        <motion.button
                            ...
                            className={`group flex w-full items-center gap-3 px-4 py-3 text-left transition-colors duration-150 ${
                                isDark ? 'hover:bg-white/5' : 'hover:bg-surface-2'
                            }`}
                        >
                            <span className={`shrink-0 font-mono text-[10px] ${isDark ? 'text-white/30' : 'text-ink-faint'}`}>
                                {String(idx + 1).padStart(2, '0')}
                            </span>
                            <span className={`size-1.5 shrink-0 rounded-full ${
                                project.isFeatured ? 'bg-accent' : (isDark ? 'bg-white/60' : 'bg-ink/40')
                            }`} />
                            <span className="min-w-0 flex-1">
                                <span className={`block truncate text-sm font-medium tracking-[-0.02em] ${
                                    isDark ? 'text-zinc-100' : 'text-ink'
                                }`}>
                                    {project.name}
                                </span>
                            ...
                            <span className={`shrink-0 text-xs opacity-40 transition-opacity group-hover:opacity-80 ${
            ...
            <motion.div
                ...
                className={`hidden md:block frame relative aspect-[4/3] ${
                    isDark ? '!bg-[#15140F] !border-white/15 text-surface-1' : 'text-ink'
                }`}
            >
                <span className="frame-corner bl" />
                    ...
                    >
                        ...
                        >
                            <span className={`size-2 rounded-full ${
                                project.isFeatured ? 'bg-accent' : (isDark ? 'bg-white/80' : 'bg-ink')
                            }`} />
                        </span>
                        <span
